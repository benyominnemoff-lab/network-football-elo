#!/usr/bin/env python3
"""Deployed opponent-network model with an evidence-adjusted public rating."""
...
class NetworkEloReplay:
    ...
    def record_values(
        ...
        The posterior mean is useful inside the forecast model, but it is not
        safely comparable across weakly connected historical eras.  In
        particular, subtracting a contemporaneous elite reference can cancel
        uncertainty shared by an isolated network.  NFELO therefore retains
        its established evidence adjustment: shrink the latent deviation by
        opponent breadth and subtract the team's *marginal* posterior
        uncertainty.  The common-mode uncertainty is intentionally preserved.
        """
    ...
    def record_for_pair(
        ...
        combined = combined_mean - CONFIDENCE_Z * pair_se
        return {
